...
export default function WhatsAppSettingsPage() {
  ...
  async function generatePokerKey() {
    setKeyLoading(true); setKeyMessage(""); setGatewayKey("");
    const { data } = await createClient().auth.getSession();
    const token = data.session?.access_token;
    if (!token) { setKeyLoading(false); return router.push("/login"); }
    const response = await fetch("https://alieniginamorau.tail01eed7.ts.net/api/poker/key", { method: "POST", headers: { Authorization: "Bearer " + token } });
    const result = await response.json().catch(() => ({}));
    setKeyLoading(false);
    if (!response.ok) return setKeyMessage(result.error || "Não foi possível gerar a chave.");
    setGatewayKey(result.apiKey);
    setKeyMessage("Chave criada. Copie agora: ela não será exibida novamente.");
  }

  async function copyPokerKey() {
    await navigator.clipboard.writeText(gatewayKey);
    setKeyMessage("Chave copiada. Cadastre-a como SALDO_AI_API_KEY no Supabase do Poker.");
  }
  ...
  return <main className="settingsPage"><section className="settingsShell"><div className="settingsTop"><Link href="/">← Voltar ao painel</Link><span className="brand"><i className="brandMark">S</i> Saldo Aí</span></div><header><div><p className="eyebrow">CONFIGURAÇÕES</p><h1>WhatsApp</h1><p>Conecte e controle o bot financeiro.</p></div><button className="secondaryButton" onClick={() => void loadStatus()}>Atualizar status</button></header>
    ...
    <article className="card commandsCard gatewayKeyCard"><h2>Chave do Presidente Poker</h2><p>Gere uma chave para cadastrar no Secret <code>SALDO_AI_API_KEY</code> do Poker. Ao gerar outra, a anterior deixa de funcionar.</p><div className="connectionActions"><button className="primary" disabled={keyLoading} onClick={() => void generatePokerKey()}>{keyLoading ? "Gerando..." : "Gerar nova chave"}</button>{gatewayKey && <button className="secondaryButton" onClick={() => void copyPokerKey()}>Copiar chave</button>}</div>{gatewayKey && <code className="gatewayKeyValue">{gatewayKey}</code>}{keyMessage && <p className={gatewayKey ? "actionMessage" : "errorMessage"} role="status">{keyMessage}</p>}</article>
  </section></main>;
}
