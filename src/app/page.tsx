...
export default function Home() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      setAccounts((accountRows ?? []).map((account) => ({ id: account.id, name: account.name, kind: account.kind, institution: account.institution, openingBalance: Number(account.opening_balance), creditLimit: account.credit_limit == null ? null : Number(account.credit_limit), statementCloseDay: account.statement_close_day, paymentDueDay: account.payment_due_day })));
  ...
  async function addAccount(event: FormEvent<HTMLFormElement>) {
    ...
    const candidate = { user_id: userId, name: String(data.get("name")), kind, institution: String(data.get("institution")) || null, opening_balance: parseAmount(String(data.get("openingBalance"))) || 0, credit_limit: kind === "credit_card" && data.get("creditLimit") ? parseAmount(String(data.get("creditLimit"))) : null, statement_close_day: kind === "credit_card" && data.get("statementCloseDay") ? Number(data.get("statementCloseDay")) : null, payment_due_day: kind === "credit_card" && data.get("paymentDueDay") ? Number(data.get("paymentDueDay")) : null };
    ...
    setAccounts((current) => [...current, { id: saved.id, name: candidate.name, kind: candidate.kind, institution: candidate.institution, openingBalance: candidate.opening_balance, creditLimit: candidate.credit_limit, statementCloseDay: candidate.statement_close_day, paymentDueDay: candidate.payment_due_day }]);
  ...
  async function importStatement(file?: File) {
    ...
    const entries = /\.ofx$/i.test(file.name) || /OFXHEADER|<OFX>/i.test(text) ? parseOfxStatement(text) : parseDelimitedStatement(text);
    const candidates = entries.slice(0, 100).map((entry) => ({ id: crypto.randomUUID(), name: entry.description, category: "A classificar", date: new Date(`${entry.occurredOn}T12:00:00`).toLocaleDateString("pt-BR"), occurredOn: entry.occurredOn, amount: entry.amount }));
    if (candidates.length === 0) return window.alert("Não encontrei lançamentos neste arquivo. Use OFX, CSV ou TXT.");
  ...
  return (
    <main className="shell">
      ...
      <section className="content">
        ...
        <article className="card accountsCard" id="accounts"><div className="cardTitle"><div><h2>Minhas contas</h2><p>Saldos, cartões e movimentações vinculadas</p></div><button onClick={() => setShowAccountForm(true)}>+ Adicionar conta</button></div><div className="accountList">{accounts.length === 0 ? <div className="emptyState compact"><strong>Nenhuma conta cadastrada</strong><p>Cadastre sua conta bancária, carteira ou cartão.</p></div> : accounts.map((account) => { const accountBalance = account.openingBalance + transactions.filter((item) => item.accountId === account.id).reduce((total, item) => total + item.amount, 0); const cardUsed = Math.abs(Math.min(accountBalance, 0)); return <div className="accountItem" key={account.id}><div><strong>{account.name}</strong><p>{account.institution || "Sem instituição"}{account.kind === "credit_card" && account.paymentDueDay ? ` • Vence dia ${account.paymentDueDay}` : ""}{account.kind === "credit_card" && account.statementCloseDay ? ` • Fecha dia ${account.statementCloseDay}` : ""}</p></div><div>{account.kind === "credit_card" && account.creditLimit != null ? <><b>{money.format(cardUsed)} usado</b><p>Disponível: {money.format(Math.max(account.creditLimit - cardUsed, 0))}</p></> : <b>{money.format(accountBalance)}</b>}</div></div>; })}</div></article>
      ...
      {showAccountForm && <div className="modalBackdrop" role="presentation" onMouseDown={() => setShowAccountForm(false)}><form className="transactionForm" onSubmit={addAccount} onMouseDown={(event) => event.stopPropagation()}><div className="formHead"><div><h2>Adicionar conta</h2><p>Para cartão, preencha também limite, fechamento e vencimento.</p></div><button type="button" onClick={() => setShowAccountForm(false)}>×</button></div><label>Nome da conta<input name="name" required placeholder="Ex.: Nubank"/></label><label>Instituição<input name="institution" placeholder="Ex.: Nu Pagamentos"/></label><div className="formGrid"><label>Tipo<select name="kind"><option value="checking">Conta-corrente</option><option value="savings">Poupança</option><option value="cash">Dinheiro</option><option value="credit_card">Cartão de crédito</option><option value="investment">Investimento</option></select></label><label>Saldo inicial<input name="openingBalance" inputMode="decimal" defaultValue="0,00"/></label></div><div className="formGrid"><label>Limite do cartão<input name="creditLimit" inputMode="decimal" placeholder="Ex.: 5.000,00"/></label><label>Fechamento da fatura<input name="statementCloseDay" type="number" min="1" max="31" placeholder="Ex.: 25"/></label></div><label>Vencimento da fatura<input name="paymentDueDay" type="number" min="1" max="31" placeholder="Ex.: 05"/></label><button className="primary formSubmit" type="submit">Criar conta</button></form></div>}
